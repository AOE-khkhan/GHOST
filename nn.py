...
# source: https://raw.githubusercontent.com/ritchie46/vanilla-machine-learning/master/vanilla_mlp.py
...
class Network:
	...
	def fit(self, x, y_true, loss, epochs, batch_size, learning_rate=1e-3, verbose=0):
		"""
		...
		#the counter to track integer epoch and the loop tracker
		c, done = 0, False
		previous_loss = 0
		while not done:
			...
			if type(epochs) == float:
				_, a = self._feed_forward(x)
				loss = self.loss.loss(y_true, a[self.n_layers])

				if verbose==0:
					print("Loss:", loss, a[3][0][0])

				if loss <= epochs or abs(previous_loss - loss) <= 1e-5:
					done = True
