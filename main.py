    ...
    for _ in range(n_iter):
        for __ in actions:
            ...
            for action in [action, 'N']:
                ret = [action] + [str(x) for x in last_position]
                last_position = position.copy()
    ...
    for training_data in td:
        for sensory_data, input_data in training_data:
            for i, state in enumerate(sensory_data):
                ...
                print('sensor_value = {}, x = {}, y = {}, y_pred = {}, weight = {}-{}, '.format(state, last_input_data, input_data, last_outputs, weight, po))
                ...
                outputs = [[str(chr(x)).encode('utf-8') for x in outputs[i]] for i in range(len(input_data))] if len(outputs) > 0 else []
                    
                last_outputs = outputs
            last_input_data = list(map(chr, data))
