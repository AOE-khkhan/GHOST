...
##            self.show_process("outputFormats_infl = {}".format(pd))
            ...
            for cl in output_classes:
                ...
                if len(sf) > 0 and len(temp_ans) > 2:
                    sfl = list(set(sf))
...
##--------------------------this is to derive the model to fetch the ans------------------------------
                    if cl == '[var]':
                        for vx in temp_ans+[data]:
                            vcl = self.getAllClasses([vx])
                            for vxn in vcl:
                                if vxn != '[var]' and vxn in cl_db:
                                    cl_db[vxn] += 1
                                else:
                                    cl_db.setdefault(vxn, 1)
##                        print(len(temp_ans), self.sort_dict(cl_db)[:10])
                ...
                print('score', temp_qna_infl, sum(temp_qna_infl), sum(qna_infl), sum(temp_qna_infl)/sum(qna_infl))
                cmps = []
                if len(cl_db) > 0:
                    for x in cl_db:
                        print('\n working on',x)
                        cmp = self.getRelated(x, sep=" ", treshold=0.1, strict=True)
##                        print('cmp', self.sort_dict(cmp)[:5])
                        cmp = {xx:self.percentage_similarity(data, x, sep=" ")*cl_db[x] for xx in cmp if self.percentage_similarity(data, x, sep=" ") > 0.1}
                        print('cmp', self.sort_dict(cmp)[:5])
                        for cx in cmp:
                            vsx = self.getRelated(cx, sep=" ", treshold=0.3, strict=True)
                            pd = self.sort_dict(vsx)
##                            print('pd', pd[:5])
##                            vsx1 = self.getRelated(cx, sep=" ", treshold=0.1, strict=True)
##                            vsx2 = self.getRelated(" ".join(cx.split()[::-1]), sep=" ", treshold=0.1, strict=True)
##
##                            l1, l2 = vsx1, vsx2
##                            if len(vsx2) < len(vsx1): l1, l2 = vsx2, vsx1
##
##                            vsx = {v:l2[v]*l1[v] for v in l1 if v in l2}
##                            pd = self.sort_dict(vsx)
                            if 'kevin mitnit is my name' in vsx:
                                print('pd', cx, vsx['kevin mitnit is my name'], pd[:2])
                            
                            if len(pd) > 0:
                                vs = [x for x in vsx if vsx[x] >= pd[0][-1]]
                            else:
                                vs = []
                            
                            for vx in vs:
                                dm =  self.mapStrings(x, vx)
                                rep = " "+" ".join([x[-1] for x in dm if x[0] == '[var]']) +" "
##                                print(" "+x.replace('[var]', '').strip()+" ",'---in----',  " "+vx+" ")
                                if " "+x.replace('[var]', '').strip()+" " in " "+vx+" " and rep not in " "+data+" " and len(rep.strip()) > 0 and rep in " "+vx+" ":
                                    print(rep, dm, x)
                                    cmps.append((rep.strip(), 1))
                print(cmps[:10])
                xx.extend(cmps)
                print()
            ...
            for pred in xx:
                if pred[0] in ansval:
                    ansval[pred[0]] += pred[-1]
                else:
                    ansval.setdefault(pred[0], pred[1])
                    
            pd = self.sort_dict(ansval)
            self.show_process("ansval = {}".format(pd))
            ...
            pd = self.sort_dict(ansval)
            self.show_process("suggested ans = {}".format(ansval))
