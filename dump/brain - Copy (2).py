...
##                        print(len(temp_ans), self.sort_dict(cl_db)[:10])
                        if vx == data:
                            common_classes = [x for x in cl_db if cl_db[x] == ans_length]
                ...
                cmps = []
                if len(cl_db) > 0:
                    data_rel = self.getRelated(data, sep=" ", treshold=0.3, strict=True)
                    for x in cl_db:
                        cmp = self.getRelated(x, sep=" ", treshold=0.1, strict=True)
                        cmp = {xx:self.percentage_similarity(data, x, sep=" ")*cl_db[x] for xx in cmp if self.percentage_similarity(data, x, sep=" ")*cl_db[x] > 0.1}
                        ...
                        for cx in cmp:
                            ...
                            for vx in vs:
                                ...
                                rep_classes = sorted(self.getAllClasses([rep]))
                                print('rep', rep_classes[:5])
                                print('cc', common_classes[:5])
                                rep = " "+rep +" "
                                if rep not in " "+data+" " and len(rep.strip()) > 0 and rep in " "+vx+" " and common_classes != [] and sorted(self.getCommon(common_classes, rep_classes)) == rep_classes:
                                    print(rep, dm)
