...
class brain_functions:
    def search_codebase(self, data, que, ans):
        cdv = {}
        ac = []
        k = self.getCommon([x.split() for x in ans])
        if(len(k) > 0):
            print(k) 
...
##            if all([xx in x and data.split().count(xx) == x.split().count(xx) for xx in data.split()]):
            for y in self.read_data(x):
                ...
                if len(y) > 0 and y == ans and sim > 0:
...
##            print('------------', len(limit_val)/length)
    ...
    def set_event(self, data, data_class,  codebase, confidence, val, key=''):
...
##            print(' not in related')
        ...
        if str(sorted(data_class)) in self.events and any([self.events_id[x] in data_rel for x in self.events[str(sorted(data_class))]]):
            ...
            self.create_event(data, str(sorted(data_class)), codebase, '0/0', val, key)
##            input('enter!')
...
##        print('increasing confidence of '+event_name, value)
        ...
        self.update_event(event_name, self.read_event(event_name, 'classes'), self.read_event(event_name, 'codebase'), str(a)+'/'+str(b), self.read_event(event_name, 'format'), self.read_event(event_name, 'key'))
##        print('increasing {} confidence to {}, {}'.format(event_name, a, b))
        
    def create_event(self, value, cls, codebase, confidence, Format, key):
    ...
    def update_event(self, event_id, cls, codebase, confidence, Format, key):
    ...
    def read_event(self, event_id, key):
        key_db = {'id':0, 'classes':1, 'codebase':2, 'confidence':3, 'format':4, 'key':5}
